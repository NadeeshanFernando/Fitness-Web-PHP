...
if ($id == 0) {
  ...
  if ($type == 'delete') {
    $sqlpic = "SELECT * FROM products WHERE id='$deltid'";
    $res    = mysqli_fetch_assoc(mysqli_query($con, $sqlpic));
    $pic  = $res['image'];

    $dltsql = "DELETE FROM products WHERE id='$deltid'";
    ...
    $edit = "UPDATE products SET status=0 WHERE id='$deltid';";
    mysqli_query($con, $edit);
    echo "<script type='text/javascript'> document.location = './products.php'; </script>";
  } elseif ($type == 'activate') {
    $edit = "UPDATE products SET status=1 WHERE id='$deltid';";
    mysqli_query($con, $edit);
    echo "<script type='text/javascript'> document.location = './products.php'; </script>";
  }
}
